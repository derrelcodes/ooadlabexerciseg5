...
public class LeftCanvasControls {
    ...
    public static JPanel createButtonPanel(LeftCanvas canvasRef, CollectionPanel collectionPanelRef) {
        ...
        // MODIFICATION: Add save options dialog
    ...
    private static void showSaveOptions() {
        ...
        int choice = JOptionPane.showOptionDialog(canvas,
                "Where would you like to save this composition?",
                "Save Composition",
                JOptionPane.YES_NO_CANCEL_OPTION,
                JOptionPane.QUESTION_MESSAGE,
                null, options, options[0]);

        if (choice == JOptionPane.YES_OPTION) { // Save to Library
            canvas.saveToLibrary(collectionPanel);
        } else if (choice == JOptionPane.NO_OPTION) { // Save to File
            canvas.saveToFile();
        }
    }
